    ...
    def walk(folder_id: str, path_parts: List[str]):
        ...
        for item in children:
            if item.get("mimeType") == "application/vnd.google-apps.folder":
                result.extend(walk(item["id"], path_parts + [item["name"]]))
            elif item.get("name", "").lower().endswith(".pdf"):
                parts = path_parts + [item["name"]]
                # Struktur yang diminta: PILEG / DAPIL / KABKOTA / KECAMATAN / PDF.
                result.append({
                    "id": item["id"],
                    "name": item["name"],
                    ...
                    "path": " / ".join(parts),
                    "provinsi": path_parts[-4] if len(path_parts) >= 4 else "^",
                    "dapil": path_parts[-3] if len(path_parts) >= 3 else "^",
                    "kab_kota": path_parts[-2] if len(path_parts) >= 2 else "^",
                    "kecamatan": path_parts[-1] if len(path_parts) >= 1 else "^",
                })
        return result
